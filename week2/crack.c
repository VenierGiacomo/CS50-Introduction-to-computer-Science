...
int main(int argc, char* argv[])
  {
    ...
    char test[9] = {};     //  array for storing passwords of all lengths

    char salt[3];          // get the salt from encrypted password
    
    salt[0] = argv[1][0];
    salt[1] = argv[1][1];

    char arr[95];         // store  ASCII characters 
    for(int i = 0; i < 95; i++) 
	arr[i] = (char) (i + 32);
