...
void reflect(int height, int width, RGBTRIPLE image[height][width])
{
    ...
    for (int j = 0; j < height; j++)
    {
        ...
        {
            tempo[0] = image[j][i].rgbtBlue;
            tempo[1] = image[j][i].rgbtGreen;
            tempo[2] = image[j][i].rgbtRed;
            ...
            image[j][width - i - 1].rgbtBlue = tempo[0];
            image[j][width - i - 1].rgbtGreen = tempo[1];
            image[j][width - i - 1].rgbtRed = tempo[2];
        }
    }
}
...
void blur(int height, int width, RGBTRIPLE image[height][width])
{
    ...
    for (int i = 0; i < width; i++)
    {
        for (int j = 0; j < height; j++)
        {
            ...
            tempo[j][i].rgbtBlue = round(sumBlue / counter);
            tempo[j][i].rgbtGreen = round(sumGreen / counter);
            tempo[j][i].rgbtRed = round(sumRed / counter);
        }
    }
    ...
    for (int i = 0; i < width; i++)
    {
        for (int j = 0; j < height; j++)
        {
            image[j][i].rgbtBlue = tempo[j][i].rgbtBlue;
            image[j][i].rgbtGreen = tempo[j][i].rgbtGreen;
            image[j][i].rgbtRed = tempo[j][i].rgbtRed;
        }
    }
}
